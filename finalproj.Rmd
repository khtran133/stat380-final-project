...
rpart(state_vcrime_rate ~ var, data = state_data %>% filter(countystat == 'poverty_rate'))
...
pca_data %>% 
  ggplot(aes(x = -PC1, y = -PC2, colour = year %>% as.factor())) + 
...
set.seed(100)
fit2 <- kmeans(df_Farm_Poverty[, 2:3], 5)
...
system.time({
  lm1 <- lm(state_vcrime_rate ~ state + year + var, 
     data = state_data %>% filter(countystat == 'poverty_rate')) %>% 
    msummary()
  lm2 <- lm(state_vcrime_rate ~ state + year + var, 
            data = state_data %>% filter(countystat == 'pop_dens')) %>% 
    msummary()
  lm3 <- lm(state_vcrime_rate ~ state + year + mean, 
            data = state_data %>% filter(countystat == 'p_farm_income')) %>% 
    msummary()
  lm4 <- lm(state_vcrime_rate ~ state + year + var, 
            data = state_data %>% filter(countystat == 'empl_rate')) %>% 
    msummary()
  lm5 <- lm(state_vcrime_rate ~ state + year + mean, 
            data = state_data %>% filter(countystat == 'income')) %>% 
    msummary()
})
