...
state_poverty <- merge(state_poverty, 
                       merge(aggregate(poverty_count ~ year + state, cp, sum), 
                         ...
                         mutate(poverty_rate = poverty_count / total_count), 
                       by = c('year', 'state'))
names(state_poverty) <- c('year', 'state', 'countystat', '1st_quantile', '3rd_quantile', 'max', 'mean', 'median', 'minimum', 'n_counties', 'state_poverty', 'state_pop', 'state_poverty_rate')
```
