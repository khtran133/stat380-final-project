...
names(state_ec) <- c('state', 'year', 'empl_count', 'farm_income', 'income', 'nonfarm_income', 'p_farm_income', 'area')
```
...
```{r}
# not the worst regressions ever tbh
lm(state_vcrime_rate ~ state %>% as.factor() + var, 
   data = state_data %>% filter(countystat == 'poverty_rate')) %>% 
  msummary()

lm(state_vcrime_rate ~ state %>% as.factor() + year %>% as.factor() + var, 
   data = state_data %>% filter(countystat == 'poverty_rate')) %>% 
  msummary()
lm(state_vcrime_rate ~ state %>% as.factor() + year %>% as.factor() + var, 
...
```{r}
# decision trees
rpart(state_vcrime_rate ~ state + year, data = state_data)
rpart(state_vcrime_rate ~ var, data = state_data %>% filter(countystat == 'poverty_rate')) %>% printcp()
rpart(state_vcrime_rate ~ ., data = state_data)
rpart(state_vcrime_rate ~ state + var, data = state_data %>% filter(countystat == 'pop_dens'))
...
state_data_pca <- state_data_pca_df[,-2] %>% 
  prcomp(scale = TRUE)
-1 * state_data_pca$rotation[, 1:2] %>% round(3)

summary(state_data_pca)

data.frame(sd = state_data_pca$sdev) %>% rownames_to_column() %>% mutate(rowname = parse_number(rowname), totalVar = sum(state_data_pca$sdev^2), pve = 100 * sd^2 / totalVar, cusum = cumsum(pve)) %>% ggplot(aes(x = rowname, y = pve)) + geom_line()

data.frame(sd = state_data_pca$sdev) %>% rownames_to_column() %>% mutate(rowname = parse_number(rowname), totalVar = sum(state_data_pca$sdev^2), pve = 100 * sd^2 / totalVar, cusum = cumsum(pve)) %>% ggplot(aes(x = rowname, y = cusum)) + geom_line()
```
...
pca_data %>% 
  ggplot(aes(x = -PC1, y = -PC2, colour = year)) + 
...
```{r}
state_data_pca$x %>% dist() %>% hclust() %>% plot(cex = 0.3)
...
pca_data_alt %>% dist() %>% hclust() %>% plot(cex = 0.8)
```
...
```{r}
# visualization
df_var1 <- filter(state_data, 
              countystat == "poverty_rate")
df_var1 <- aggregate(var ~ state, data = state_data, mean)
df_var1 <- filter(df_var1, 
                  state != "DC") 
mycolors_True <- brewer.pal(9, "GnBu")
states <- tolower(rownames(USArrests))
map_50S <- map_data("state")
ggplot(data = df_var1,
       aes(fill = var)) +
  labs(x = "Longitude", y = "Latitude", title = "Variance of Poverty Rate in Counties, Per State") +
  geom_map(aes(map_id = states), map = map_50S) +
  expand_limits(x = map_50S$long, y = map_50S$lat) +
  theme_bw() + 
...
```{r}
# visualization
df_var2 <- filter(state_data, 
              countystat == "pop_dens")
df_var2 <- aggregate(var ~ state, data = state_data, mean)
df_var2 <- filter(df_var2, 
                  state != "DC") 
mycolors_True <- brewer.pal(9, "GnBu")
states <- tolower(rownames(USArrests))
map_50S <- map_data("state")
ggplot(data = df_var2,
       aes(fill = var)) +
  labs(x = "Longitude", y = "Latitude", title = "Variance of Population Density in Counties, Per State") +
  geom_map(aes(map_id = states), map = map_50S) +
  expand_limits(x = map_50S$long, y = map_50S$lat) +
  theme_bw() + 
  scale_fill_gradientn(name = "Variance", colours = mycolors_True,
